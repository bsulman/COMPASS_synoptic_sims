...
import sys
sys.path.append('/ccsopen/home/b0u/ELM-alquimia/REDOX-PFLOTRAN')
import plot_ELM_alquimia_result as pltELM

yr=1994
COMPASS=xarray.open_dataset(f'/gpfs/wolf2/cades/cli185/scratch/b0u/COMPASS/COMPASS_synoptic_US-GC3_ICB20TRCNRDCTCBC.elm.h0.{yr:d}-01-01-00000.nc')

from make_COMPASS_ELM_forcing import grid_points

vars=['VWC','O2','salinity','DOC','FeOxide','Fe2','FeS','Sulfate','Sulfide','pH']
...
for site in ['Moneystump','Crane Creek']:
    ...
    pltELM.plot_vars(COMPASS.isel(lndgrid=grid_points.index(site+'_Wetland')),vars,vmax=vmax,vmin=vmin,a_contour=a[:,0],a_profile=a[:,3],profile_color='blue',mean_profile=True,quantiles=[0.1,0.9],maxdepth=0.45,time_format='%b')
    pltELM.plot_vars(COMPASS.isel(lndgrid=grid_points.index(site+'_Transition')),vars,vmax=vmax,vmin=vmin,a_contour=a[:,1],a_profile=a[:,3],profile_color='purple',mean_profile=True,quantiles=[0.1,0.9],maxdepth=0.45,time_format='%b')
    pltELM.plot_vars(COMPASS.isel(lndgrid=grid_points.index(site+'_Upland')),vars,vmax=vmax,vmin=vmin,a_contour=a[:,2],a_profile=a[:,3],profile_color='red',mean_profile=True,quantiles=[0.1,0.9],maxdepth=0.45,time_format='%b')
